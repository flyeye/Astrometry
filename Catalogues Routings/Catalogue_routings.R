...
HRDiagram <- function(data)
{
  ...
  g <- ggplot() + geom_point(data=hrdata, aes(x = hrdata$B_V, y = hrdata$M), alpha = 0.05, na.rm = TRUE, size = 0.1) + scale_y_reverse(breaks=seq(10,-10,by=-2), minor_breaks=seq(10,-10,by=-1), limits = c(10,-10)) + scale_x_continuous(breaks=seq(-1,3,by=1), minor_breaks=seq(-1,3,by=0.5), limits = c(-1,3)) + xlab("B-V") + ylab("M") + ggtitle("HertzsprungЦRussell")  
