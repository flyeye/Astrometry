...
calc_physical_params <- function(solution, Rs = 8.09)
{
  ...
  physical[,1] <- Rs * (solution$Oort[,1] - solution$Oort[,2]) # Vs - линейна€ скорость вращени€ галактики на рассто€нии Rs
  physical[,2] <- (2 * pi * Rs * 3.086e+16 /  physical[,1]) / (86400*365*1000000)   # P - период вращени€ галактики
  physical[,3] <- solution$Oort[,1] + solution$Oort[,2]        # S - наклон кривой вращени€ галактикки
  physical[,4] <- 2 * sqrt(-solution$Oort[,2]/(solution$Oort[,1] - solution$Oort[,2])) # отношение эпициклической частоты к угловой скорости вращени€ √алактики в окресност€х —олнца
  physical[,5] <- (Rs*3.086e+19) * ((physical[,1]*1000)**2) / 132712438e+12 #6.67408e-11           # масса вещества галактики, сосредоточенна€ внутри орбиты —олнца
  ...
  physical[,8] <- sqrt(solution$X[,1]**2 + solution$X[,2]**2 + solution$X[,3]**2) # скоросто —олнца
...
tgas_draw_all_kinematic_comp <- function(solutions, src = "TGAS", saveto = "")
{
  ...
  if ((solutions[[1]]$Conditions$KinModel==2) & (solutions[[1]]$Conditions$KinModelType==2))
  {
    g <- draw_OortParameter(solutions, 
                            parameter = 5,
                            title = paste("Oort`s parameter Gx, ", src, " proper motions."),
                            x_lim = c(0, 4, 0.5), y_lim = c(-40, 40, 2))
    ggsave(paste0(save, "OortGx-R", ".png"), plot = g, width = 10, height = 5)
    ggsave(paste0(save, "OortGx-R", ".eps"), plot = g, width = 10, height = 5)
    
    g <- draw_OortParameter(solutions, 
                            parameter = 6,
                            title = paste("Oort`s parameter Gy, ", src, " proper motions."),
                            x_lim = c(0, 4, 0.5), y_lim = c(-40, 40, 3))
    ggsave(paste0(save, "OortGy-R", ".png"), plot = g, width = 10, height = 5)
    ggsave(paste0(save, "OortGy-R", ".eps"), plot = g, width = 10, height = 5)  
  }
}
...
read_ucac4 <- function(path, start = 1, n = Inf, is_tyc_only = TRUE)
{
  ...
  if(is_tyc_only)
  {
    ...
    # то есть на один TYC может приходитс€ несколько уникальных записей в UCAC4. ¬ылон€€ ниже left_join
    # мы дублируем записи TGAS, дополеннные разными источниками из UCAC4. ¬се бы ничего, но TYC и TGAS source_id 
    # тер€ют при этом уникальность и в последующем при дальнейших оперци€х сл€ини€ количество дублей будет множитьс€. 
    # поэтому по хорошему тут нужно все дубли беспощадно выкинуть. ¬се равно пока другого способа идентификации звезд
    # TGAS и APASS нет кроме как по TYC, а он вот такой ущербный, соответственно даже если в TGAS и APASS 
    # соответствующие звезд разделены, св€зать мы их пока никак не можем. 
